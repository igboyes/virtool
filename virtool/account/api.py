...
async def edit(req):
    ...
    if password is not None:
        ...
        if not await virtool.users.db.validate_credentials(
            db, user_id, old_password or ""
        ):
...
async def update_settings(req):
    ...
    settings_from_db = await virtool.db.utils.get_one_field(
        db.users, "settings", user_id
    )
...
async def create_api_key(req):
    ...
    headers = {"Location": f"/api/account/keys/{document['id']}"}
...
async def update_api_key(req):
    ...
    if not user["administrator"]:
        permissions = virtool.users.utils.limit_permissions(
...
async def login(req):
    ...
    if await virtool.db.utils.get_one_field(db.users, "force_reset", user_id):
        return json_response(
            {
                "reset": True,
                "reset_code": await virtool.users.sessions.create_reset_code(
...
async def reset(req):
    ...
    # Update the user password and disable `force_reset`.
